...
public class UserServiceImpl extends ServiceImpl<UserMapper, User>  implements UserService{
    ...
    public long userRegister(String userAccount, String userPassword, String checkPassword) {
        log.info("开始用户注册，账号：{}", userAccount);
        // 判断用户账号是否为空
        if (StringUtils.isBlank(userAccount)){
            log.warn("用户注册失败：用户账号为空");
        ...
        if (StringUtils.isBlank(userPassword) || userPassword.length() < 8){
            log.warn("用户注册失败：密码为空或长度不足，账号：{}", userAccount);
            throw new BusinessException(ErrorCode.PARAMS_ERROR,"用户密码不能为空,且密码大小不能小于8位");
        }
        // 确认两次密码是否一致
        if (!userPassword.equals(checkPassword)){
            log.warn("用户注册失败：两次密码不一致，账号：{}", userAccount);
        ...
        if (count > 0){
            log.warn("用户注册失败：用户已存在，账号：{}", userAccount);
       ...
       if (!saveResult){
           log.error("用户注册失败：数据库保存失败，账号：{}", userAccount);
           throw new BusinessException(ErrorCode.SYSTEM_ERROR,"数据库异常");
       }
       log.info("用户注册成功，用户ID：{}，账号：{}", user.getId(), userAccount);
        return user.getId();
    }
    ...
    public LoginUserVO userLogin(String userAccount, String userPassword, HttpServletRequest request) {
        log.info("开始用户登录，账号：{}", userAccount);
        if (StringUtils.isBlank(userAccount)){
            log.warn("用户登录失败：账号为空");
            throw new BusinessException(ErrorCode.PARAMS_ERROR,"用户账号不能为空");
        }
        if (StringUtils.isBlank(userPassword)){
            log.warn("用户登录失败：密码为空，账号：{}", userAccount);
        ...
        if (user == null){
            log.warn("用户登录失败：用户不存在或密码错误，账号：{}", userAccount);
            throw new BusinessException(ErrorCode.PARAMS_ERROR,"用户不存在或密码错误");
        }
        request.getSession().setAttribute(USER_LOGIN_STATE,user);
        log.info("用户登录成功，用户ID：{}，账号：{}", user.getId(), userAccount);
    ...
    public User getLoginUser(HttpServletRequest request) {
        ...
        if (currentUser == null) {
            log.warn("获取登录用户失败：用户不存在，用户ID：{}", userId);
            throw new BusinessException(ErrorCode.NOT_LOGIN_ERROR, "当前未登录");
        }
        log.debug("获取登录用户信息成功，用户ID：{}", userId);
        return currentUser;
    }
    @Override
    ...
    public boolean userLogout(HttpServletRequest request) {
        ...
        log.info("用户注销，用户ID：{}", currentUser.getId());
        //移除登录态
    ...
    public QueryWrapper getQueryWrapper(UserQueryRequest userQueryRequest) {
        ...
        log.debug("构建用户查询条件，账号：{}，用户名：{}，角色：{}", userAccount, userName, userRole);
